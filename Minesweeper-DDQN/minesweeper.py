...
class Minesweeper:
    ...
    def flag_tile(self, action):
        # Convert flat action index to 2D coordinates
        idx_x, idx_y = np.unravel_index(action, (self.rowdim, self.coldim))
        # Only allow flagging hidden tiles
        if self.playerfield[idx_x, idx_y] == 9:
            # If already flagged, unflag it
            if (idx_x, idx_y) in self.flagged_tiles:
                self.flagged_tiles.remove((idx_x, idx_y))
                self.mines_remaining += 1
                # Update playerfield to show unflagged (hidden) tile
                self.playerfield[idx_x, idx_y] = 9
            else:
                # Flag the tile
                self.flagged_tiles.add((idx_x, idx_y))
                self.mines_remaining -= 1
                # Update playerfield to represent flagged tile (you could use value 10)
                self.playerfield[idx_x, idx_y] = 10  # New value for flagged tiles
        return self.playerfield, 0, False  # Return state, no reward, not done
    ...
    def step(self, action):
        ...
        if state[action] == -1:
            ...
            if num_hidden_tiles == self.mine_count:
                ...
                # Add reward shaping based on local context
                # Get the coordinates of the selected tile
                idx_x, idx_y = np.unravel_index(action, (self.rowdim, self.coldim))
                
                # Count adjacent revealed tiles before this action
                adjacent_revealed = 0
                for i in range(max(0, idx_x-1), min(self.rowdim, idx_x+2)):
                    for j in range(max(0, idx_y-1), min(self.coldim, idx_y+2)):
                        if self.playerfield[i,j] != 9 and (i != idx_x or j != idx_y):
                            adjacent_revealed += 1
                
                # Higher reward for expanding from known areas
                if adjacent_revealed > 0:
                    reward += 0.05 * adjacent_revealed
        else:
            # Player has revealed a non-mine tile, but has not won yet
            done = False
            reward = 0.1
            ...
            for i in range(max(0, idx_x-1), min(self.rowdim, idx_x+2)):
                for j in range(max(0, idx_y-1), min(self.coldim, idx_y+2)):
                    # Only count tiles that were already revealed before this action
                    if state.reshape(self.rowdim, self.coldim)[i,j] != 9 and (i != idx_x or j != idx_y):
                        adjacent_revealed += 1
