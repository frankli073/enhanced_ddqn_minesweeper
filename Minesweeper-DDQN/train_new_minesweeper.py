...
from env_wrappers import MineCountWrapper
...
base_env = Minesweeper(ROWDIM, COLDIM, MINE_COUNT)
env      = MineCountWrapper(base_env)          # <‑‑ NEW

# state     = env.reset()
# state_dim = state.shape[0]                     # 81 + 1 = 82
# action_dim = env.rowdim * env.coldim           # unchanged

# agent = DDQN(state_dim, action_dim, ...)       # build the net
